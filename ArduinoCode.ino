 ...
 * of resigning myself to a shitty naming scheme, I moved all pin constants here. You can just fold the code down.
 */
...
const int stepPinY = 39;
const int dirPinY = 40;
const int enablePinY = 41;
const int stepPinX = 45;
const int dirPinX = 46;     //LOW is clockwise
const int enablePinX = 47;
 ...
 */
Servo nozzleServo;
const String SERVO_TYPE = "CONTINUOUS";
/**
 * Amount of time (in uS) the servo takes for a 180 degree rotation, should be changed depending on end servo.
 */
const int timeForCompleteRotation = 2000;
 ...
 */
AccelStepper xMotor(AccelStepper::DRIVER, stepPinX, dirPinX);
AccelStepper yMotor(AccelStepper::DRIVER, stepPinY, dirPinY);
 ...
 */
void loop()  {
	if ((xMotor.distanceToGo() == 0) && (yMotor.distanceToGo() == 0) && (taskIsExecuting == true)){
